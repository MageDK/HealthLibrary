 ...
 * The can be used to reduce the number of requests that need to be made to retrieve remote content.  
 * The cache uses a data id to identify each cache entry.  It is up to the caller to generate 
 * unique data id's for each item that we want to cache.  The easiest method for this is using a 
 * Hash function to combine the identifying information of the request into a single unique value.
 * 
...
class FileCache
{
	...
	/**
	 * @var integer The number of secids befire an entry in the cache is expired.  Default value is 900 (15 minutes).
	 */
	...
	/**
	 * Set the directory to use to store our cache data.  This function will check to make sure 
	 * the directory exists and is writable.
	 * 
	 * @param string $sDirectory The path to the directory we want to use to store our cache.
	 * 
	 * @return boolean Returns TRUE if the specified directory was successfully set.  
	 */
	 ...
	 * @param string $sDataId The Unique ID of the data we want to check the cache for.
	 * 
	 * @return boolean If there is a entry in the cache for it then return TRUE, else return FALSE.
	 */
	 ...
	 * in the cache and is not expired.  If the content has expired then the file is deleted from 
	 * the cache directory.
	 * 
	 * @param string $sDataId The Unique ID of the data we want to check the cache for.
	 * 
	 * @return boolean Returns TRUE if the content is in the cace and has not expired.  FALSE if the content is not in the cance or expired.
	 */
	...
	/**
	 * Write the data to the file cache folder.
	 * 
	...
	/**
	 * Read the data specified by the specified sDataId from the file cache.
	 * 
	 * 
